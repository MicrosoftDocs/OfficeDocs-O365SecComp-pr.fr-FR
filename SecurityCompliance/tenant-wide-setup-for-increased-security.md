...
|***Authentification du courrier électronique*** <br/> |Oui  <br/> |Authentification de messagerie utilise un système DNS (Domain Name System) pour ajouter des informations vérifiables pour les messages électroniques à propos de l’expéditeur d’un message électronique. Office 365 configure l’authentification de courrier électronique pour son domaine par défaut (onmicrosoft.com), mais les administrateurs Office 365 peut également utiliser l’authentification du courrier électronique pour les domaines personnalisés. Trois méthodes d’authentification sont utilisées :<br/> <br> &ensp;&ensp;• Sender Policy Framework (ou SPF). Pour le programme d’installation, voir [Set up SPF dans Office 365 afin d’empêcher l’usurpation d’identité](set-up-spf-in-office-365-to-help-prevent-spoofing.md).<br/> &ensp;&ensp;• DomainKeys identifié Mail (DKIM). Pour utiliser DKIM, voir [Utiliser les DKIM pour valider le courrier sortant envoyé à partir de votre domaine personnalisé dans Office 365](https://docs.microsoft.com/office365/SecurityCompliance/use-dkim-to-validate-outbound-email).<br>&ensp;&ensp;&ensp;&ensp;&ensp;-Une fois que vous avez configuré DKIM, vous pouvez l’activer pour votre organisation de la sécurité &amp; centre de conformité.<br/> &ensp;&ensp;• Authentification de Message basé sur le domaine, création de rapports et de conformité (DMARC). Pour configurer les DMARC, voir [Utiliser DMARC pour valider le courrier dans Office 365](use-dmarc-to-validate-email.md).<br/>  <br/>
|
