...
> [!TIP]
> Pour rechercher les valeurs qui contiennent des espaces, utilisez des guillemets doubles (« ?? ») contenant l’expression ; par exemple, `businessaddress:"123 Main Street"`. 
...
|To  <br/> |Le destinataire d’un message électronique.  <br/> |
...
Alors que la `ViewableByExternalUsers` propriété représente l’état de si un document ou un site est partagé avec des utilisateurs externes, il existe certaines restrictions à ce que cette propriété n’et refléter doesn???t. Dans les scénarios suivants, la valeur de la `ViewableByExternalUsers` propriété ne sont pas mis à jour et les résultats d’une requête de recherche de contenu utilisant cette propriété peuvent être inexactes. 
