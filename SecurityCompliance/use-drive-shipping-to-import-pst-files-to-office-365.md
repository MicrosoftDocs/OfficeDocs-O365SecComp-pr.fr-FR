...
- Vous devez stocker les fichiers PST que vous souhaitez copier sur un disque dur sur un serveur de fichiers ou un dossier partagé de votre organisation. À l'étape 2, vous exécuterez l'outil Azure Import Export Tool (outil waimportexport. exe) qui copiera les fichiers PST stockés sur ce serveur de fichiers ou dossier partagé sur le disque dur.
...
3. Exécutez la commande suivante lorsque vous utilisez l’outil WAImportExport.exe pour la première fois pour copier les fichiers PST sur un disque dur.
  ...
  <br/> Le personnel du centre de données Microsoft utilise les informations du fichier journal pour associer le disque dur au travail d'importation que vous créez à l'étape 4, et pour télécharger les fichiers PST dans la zone de stockage Azure du Cloud Microsoft.  <br/> | `/j:PSTHDD1.jrn` <br/> |
    ...
    | `/srcdir:` <br/> |Spécifie le répertoire source de votre organisation qui contient les fichiers PST qui seront copiés au cours de la session. Veillez à entourer la valeur de ce paramètre de guillemets doubles ("").  <br/> | `/srcdir:"\\FILESERVER01\PSTs"` <br/> |
    | `/dstdir:` <br/> |Spécifie le répertoire de destination dans la zone de stockage Azure du Cloud Microsoft où les fichiers PST seront téléchargés. Vous devez utiliser la valeur `ingestiondata/`. Veillez à entourer la valeur de ce paramètre de guillemets doubles ("").<br/> Vous pouvez également ajouter un chemin d'accès de fichier supplémentaire à la valeur de ce paramètre. Par exemple, vous pouvez utiliser le chemin d'accès au fichier du répertoire source sur le disque dur (converti au format URL), qui est spécifié dans `/srcdir:` le paramètre. Par exemple, `\\FILESERVER01\PSTs` est remplacé par `FILESERVER01/PSTs`. Dans ce cas, vous devez toujours inclure `ingestiondata` dans le chemin d'accès au fichier. Par conséquent, dans cet exemple, la valeur `/dstdir:` du paramètre serait `"ingestiondata/FILESERVER01/PSTs"`.<br/> L'une des raisons pour ajouter le chemin d'accès au fichier supplémentaire est si vous avez des fichiers PST portant le même nom de fichier.  <br/> > [!NOTE]> si vous incluez le chemin d'accès facultatif, l'espace de noms d'un fichier PST après son chargement dans la zone de stockage Azure inclut le chemin d'accès et le nom du fichier PST; par exemple, `FILESERVER01/PSTs/annb.pst`. Si vous n'incluez pas de chemin d'accès, l'espace de noms est uniquement le nom de fichier PST; par exemple `annb.pst`.           | `/dstdir:"ingestiondata/"` <br/> Ou  <br/>  `/dstdir:"ingestiondata/FILESERVER01/PSTs"` <br/> |
    | `/sk:` <br/> |Spécifie la clé de compte de stockage que vous avez obtenue à l'étape 1. Veillez à entourer la valeur de ce paramètre de guillemets doubles ("").  <br/> | `"yaNIIs9Uy5g25Yoak+LlSHfqVBGOeNwjqtBEBGqRMoidq6/e5k/VPkjOXdDIXJHxHvNoNoFH5NcVUJXHwu9ZxQ=="` <br/> |
    | `/encrypt` <br/> |Ce commutateur active BitLocker pour le disque dur. Ce paramètre est obligatoire la première fois que vous exécutez l’outil WAImportExport.exe.  <br/> La clé de chiffrement BitLocker est copiée dans le fichier journal et dans le fichier journal créé si vous `/logfile:` utilisez le paramètre. Comme expliqué précédemment, le fichier journal est enregistré dans le dossier où se trouve l'outil outil waimportexport. exe.<br/> | `/encrypt` <br/> |
    | `/logdir:` <br/> |Ce paramètre facultatif spécifie un dossier dans lequel enregistrer les fichiers journaux. Si ce n'est pas spécifié, les fichiers journaux sont enregistrés dans le dossier où se trouve l'outil outil waimportexport. exe. Veillez à entourer la valeur de ce paramètre de guillemets doubles ("").  <br/> | `/logdir:"c:\users\admin\desktop\PstImportLogs"` <br/> |
...
12. Une fois le nom du fichier CSV affiché sous **nom du fichier de mappage**, cliquez sur **valider** pour vérifier que votre fichier CSV contient des erreurs. 
    ...
    Le fichier CSV doit être validé correctement pour créer une tâche d'importation PST. Remarque le nom de fichier est changé en vert une fois qu'il a été validé. Si la validation échoue, cliquez sur le lien **afficher le journal** . Un rapport d'erreur de validation est ouvert, avec un message d'erreur pour chaque ligne du fichier ayant échoué. 
...
- Expédiez le disque dur à l'adresse de l'emplacement Microsoft qui s'affichait lorsque vous avez créé le travail d'importation à l'étape 4. Veillez à inclure «service d'importation Office 365» dans l'adresse de livraison.
...
3. **Numéro de compte** de l'opérateur de retour Tapez le numéro de compte de votre organisation pour le transporteur qui apparaît sous l' **opérateur de retour**. Microsoft utilisera (et facturera) ce compte pour vous remettre votre disque dur. Notez que les organisations aux États-Unis et en Europe doivent disposer d'un compte auprès de FedEx. Les organisations en Asie et dans le reste du monde doivent disposer d'un compte auprès d'DHL.
    
6. Cliquez sur **Enregistrer** pour enregistrer ces informations pour la tâche d’importation. 
    
    Sur la page d' **importation** , ![cliquez sur](media/O365-MDM-Policy-RefreshIcon.gif) actualiser l'icône **Actualiser** pour mettre à jour les informations pour votre tâche d'importation d'envoi de lecteurs. Notez que Status est maintenant défini sur **Drives in transit**.
...
7. Si vous avez choisi d'importer toutes les données, cliquez sur **importer des données** pour démarrer le travail d'importation. 
    ...
    L'état de la tâche d'importation est affiché sur la page d' **importation** . Cliquez ![sur Actualiser l'icône](media/O365-MDM-Policy-RefreshIcon.gif) **Actualiser** pour mettre à jour les informations d'État affichées dans la colonne **État** . Cliquez sur le travail d'importation pour afficher la page de menu volant d'État, qui affiche des informations sur l'état de chaque fichier PST importé. Lorsque l'importation est terminée et que les fichiers PST ont été importés dans des boîtes aux lettres utilisateur, l'État devient **terminé**.
...
7. Une fois l'URL affichée, copiez-la et enregistrez-la dans un fichier. Veillez à copier l'URL complète.
