...
La prévention de perte de données (DLP) dans Office 365 intègre de nombreux [types d’informations sensibles](what-the-sensitive-information-types-look-for.md) prêts à l’emploi dans vos stratégies DLP. Ces types intégrés permettent d’identifier et de protéger les numéros de carte de crédit, les numéros de compte bancaire, les numéros de passeport et bien plus encore. 
  
Cependant, si vous devez identifier et protéger un autre type d’informations sensibles (par exemple, un ID d’employé qui utilise un format spécifique de votre organisation), vous pouvez créer un type d’informations sensibles personnalisé. Un type d’informations sensibles est défini dans un fichier XML appelé package de règles.
...
Notez que dans le Centre de sécurité &amp; conformité, vous utilisez la cmdlet `DlpSensitiveInformationTypeRulePackage` pour charger un package de règles. Auparavant, dans le Centre d’administration Exchange, vous utilisiez la cmdlet `ClassificationRuleCollection`. 
...
2. [Connectez-vous au Centre de sécurité &amp; conformité Office 365 à l’aide de PowerShell à distance](http://go.microsoft.com/fwlink/?LinkID=799771&amp;clcid=0x409).
    
3. Dans le Centre de sécurité &amp; conformité PowerShell, saisissez New-DlpSensitiveInformationTypeRulePackage -FileData (Get-Content -Path "C:\custompath\MyNewRulePack.xml" -Encoding Byte).
    
    Assurez-vous d’utiliser l’emplacement de fichier où est stocké votre pack de règles. C:\custompath\ est un espace réservé.
    
4. Pour confirmer, saisissez Y, puis appuyez sur ENTRÉE.
    
5. Saisissez Get-DlpSensitiveInformationType pour vérifier que votre nouveau type d’informations sensibles a été chargé et afficher une liste de tous les types sensibles. Vous pouvez rapidement distinguer les types d’informations sensibles personnalisés des types intégrés en consultant la colonne Éditeur. Vous pouvez filtrer la liste sur un type d’informations sensibles particulier en exécutant Get-DlpSensitiveInformationType -Identity "name of sensitive information type".
...
1. [Connectez-vous au Centre de sécurité &amp; conformité Office 365 à l’aide de PowerShell à distance](http://go.microsoft.com/fwlink/?LinkID=799771&amp;clcid=0x409).
    
2. Dans le Centre de sécurité &amp; conformité PowerShell, effectuez l’une des opérations suivantes :
    
  - Pour supprimer un package de règles entier et toutes les entités qu’il contient
    
    Saisissez Remove-DlpSensitiveInformationTypeRulePackage "NameOfYourRulePack" ; pour l’exemple XML indiqué ci-dessus, saisissez Remove-DlpSensitiveInformationTypeRulePackage "Employee ID Custom Rule Pack".
    
    Notez que pour identifier votre package de règles, vous pouvez utiliser l’élément \<Nom\> (pour n’importe quelle langue) dans l’élément \<Pack de règles\> ou le GUID de l’attribut id de l’élément RulePack.
    
  - Pour supprimer une seule entité d’un package de règles
    
    Vous devez charger une nouvelle version de votre package de règles avec l’entité supprimée en question à l’aide de Set-DlpSensitiveInformationTypeRulePackage. Vous devez vous assurer qu’aucune stratégie DLP ou règle de transport Exchange ne référence le type d’informations sensibles avant de supprimer l’entité.
    
3. Pour confirmer, saisissez Y, puis appuyez sur ENTRÉE.
    
4. Saisissez Get- DlpSensitiveInformationType, pour vérifier que votre nouvelle règle a été supprimée. Le nom de votre type d’informations sensibles ne devrait plus s’afficher.
