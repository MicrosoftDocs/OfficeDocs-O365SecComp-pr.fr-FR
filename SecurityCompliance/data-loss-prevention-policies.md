...
- **Restreindre l’accès au contenu** Pour le contenu de site, cela signifie que les autorisations du document sont restreintes pour tout le monde sauf pour l’administrateur principal de la collection de sites, le propriétaire du document et la personne qui a modifié le document pour la dernière fois. Ces personnes peuvent supprimer les informations sensibles du document ou effectuer d’autres actions correctives. Lorsque le document est en conformité, les autorisations d’origine sont automatiquement restaurées. Lorsque l’accès à un document est bloqué, le document apparaît avec une icône de conseil de stratégie spécifique dans la bibliothèque sur le site. 
    
    ![Conseil de stratégie montrant que l’accès au document est bloqué](media/b6cefed3-d212-43d7-8534-4b92b26ebd50.png)
  
    Pour le contenu de messagerie, cette action bloque l’envoi du message. En fonction de la configuration de la règle DLP, l’expéditeur voit un rapport de non-remise ou (si la règle utilise une notification) un Conseil de stratégie et/ou une notification par courrier électronique.
...
Lorsque vous créez des règles dans une stratégie, une priorité est affectée à chaque règle, dans l’ordre dans lequel elles ont été créées, c’est-à-dire que la règle créée en premier a la priorité, la règle créée en second a la deuxième priorité, et ainsi de suite. 
...
- Trop de contenu qui **ne contient pas** d’informations sensibles correspond aux règles (en d’autres termes, trop de faux positifs). 
...
Le nombre d’instances signifie simplement le nombre d’occurrences d’un type spécifique d’informations sensibles qui doivent être présentes pour que le contenu corresponde à la règle. Par exemple, le contenu correspond à la règle indiquée ci-dessous si entre 1 et 9 (États-Unis ou Royaume-Uni). les numéros de passeport sont identifiés.
...
- S’il existe des correspondances pour plusieurs modèles (autrement dit, il existe des correspondances avec deux niveaux de confiance différents), un niveau de confiance supérieur à celui des modèles uniques est renvoyé. Il s’agit de la partie délicate. Par exemple, pour une carte de crédit, si les modèles 65% et 85% sont mis en correspondance, le niveau de confiance renvoyé pour ce type d’informations sensibles est supérieur à 90%, car davantage de preuves signifient plus de confiance.
...
Vous ne pouvez actuellement utiliser qu’une étiquette de rétention en tant que condition, et non une [étiquette de critère de diffusion](sensitivity-labels.md). Nous travaillons actuellement sur la prise en charge de l’utilisation d’une étiquette de sensibilité dans cette condition.
