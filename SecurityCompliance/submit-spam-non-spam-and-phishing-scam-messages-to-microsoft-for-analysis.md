...
Les administrateurs peuvent examiner davantage d’informations sur les paramètres du courrier indésirable qui s’appliquent à une organisation entière. Découvrez [Comment vous assurer qu’un message n’est pas marqué comme courrier](prevent-email-from-being-marked-as-spam.md)indésirable. Ces informations sont utiles si vous disposez d'un contrôle de niveau administrateur et si vous souhaitez éviter les faux positifs ou les faux négatifs.
