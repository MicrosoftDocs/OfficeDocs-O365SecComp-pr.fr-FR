...
Utilisez le flux de travail suivant pour corriger les fichiers avec des erreurs dans les cas avancés de découverte électronique (aperçu).
...
1. Sous l'onglet **traitement** dans un cas avancé EDiscovery (aperçu), sélectionnez **Erreurs** dans le menu déroulant **affichage** .
...
8. À présent, revenez à Advanced eDiscovery (Preview) et cliquez sur **Next: upload files**.  Cette opération passe à l'étape suivante, dans laquelle vous pouvez à présent télécharger les fichiers.
...
11. Enfin, revenez à Advanced eDiscovery (Preview) et cliquez sur **Next: process files**.
...
Pour obtenir une définition de tous les champs de métadonnées de document dans Advanced eDiscovery (Preview), voir [Field Metadata Fields](document-metadata-fields.md).