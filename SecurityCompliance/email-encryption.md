...
Le chiffrement est le processus par lequel les informations sont codées afin que seul un destinataire autorisé puisse décoder et consommer les informations. Office 365 utilise le chiffrement de deux manières: dans le service et en tant que contrôle client. Dans le service, le chiffrement est utilisé dans Office 365 par défaut; vous n'êtes pas obligé de configurer quoi que ce soit. Par exemple, Office 365 utilise le protocole TLS (Transport Layer Security) pour chiffrer la connexion, ou session, entre deux serveurs. 
...
Pour plus d'informations sur les options de chiffrement de courrier électronique pour votre abonnement Office 365, consultez la rubrique [Description du service Exchange Online](https://technet.microsoft.com/en-us/library/exchange-online-service-description.aspx). Vous trouverez ici des informations sur les fonctionnalités de chiffrement suivantes:
...
«Données sur REST» fait référence à des données qui ne sont pas activement en transit. Dans Office 365, les données de messagerie électronique au repos sont chiffrées à l'aide du chiffrement de lecteur BitLocker. BitLocker chiffre les disques durs des centres de 365 Office pour fournir une protection améliorée contre les accès non autorisés. Pour en savoir plus, consultez la rubrique [vue d'ensemble de BitLocker](https://go.microsoft.com/fwlink/p/?LinkId=394737).
