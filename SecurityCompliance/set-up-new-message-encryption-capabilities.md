...
Pour une liste détaillée des différences entre la version précédente d’OME et les nouvelles fonctionnalités OME, voir [comparer les versions d’OME](ome-version-comparison.md).
...
Les nouvelles fonctionnalités OME sont automatiquement activées pour la plupart des organisations Office 365. Si votre organisation ne possède pas automatiquement OME activé, ou si vous avez activé les nouvelles fonctionnalités OME désactivé, procédez comme suit pour configurer manuellement les nouvelles fonctionnalités pour OME.
...
2. Exécutez l’applet de commande Test-IRMConfiguration à l’aide de la syntaxe suivante :
   ...
   Par exemple :
    ...
    Où l’adresse est l’adresse de messagerie d’un utilisateur dans votre organisation Office 365. Facultatif, qui fournit une adresse de messagerie de l’expéditeur force le système à effectuer des contrôles supplémentaires. Vos résultats doivent se présenter comme celles-ci :
